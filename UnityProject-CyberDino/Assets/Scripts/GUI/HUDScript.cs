...
public class HUDScript : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		if(Application.loadedLevelName == "DumbellTrack")
			mapInfo = new MapGUIInfo(51.0f, new Rect(87.4f, 58.5f, 1, 1), new Rect(78, 28, 20, 60), (Texture)Resources.Load("GUI/Materials/DumbBellmap"));
		else
		...
		//mapGfx = new Texture[1];
		//mapGfx[0] = (Texture)Resources.Load("GUI/Materials/DumbBellmap");
		...
		playerNamesObjs[0] = CreateGUIText("Player 1 Name", "player 1", new Vector3(0, 0, 0));
		playerNamesObjs[1] = CreateGUIText("Player 2 Name", "player 2", new Vector3(0, 0, 0));
		playerNamesObjs[2] = CreateGUIText("Player 3 Name", "player 3", new Vector3(0, 0, 0));
		playerNamesObjs[3] = CreateGUIText("Player 4 Name", "player 4", new Vector3(0, 0, 0));
	...
	private Rect GetHealth(Rect _pos, int _index)
	{
		...
		healthBarObjs[_index].guiTexture.texture = (_pos.width / 3) < percentResults ? (Texture)Resources.Load("GUI/Materials/HealthBar") : (Texture)Resources.Load("GUI/Materials/HealthBarRed"); 
	...
	private void AttachHealth(int _index)
	{
		...
		if(screenPoint.z > 20 && screenPoint.z < 250 && playerNamesObjs[_index].guiText.fontSize < 1000)
		{
			healthBorderObjs[_index].guiTexture.pixelInset = new Rect(screenPoint.x, screenPoint.y, 5000 / screenPoint.z, 1000 / screenPoint.z);
			healthBarObjs[_index].guiTexture.pixelInset = GetHealth(new Rect(screenPoint.x, screenPoint.y, 5000 / screenPoint.z, 1000 / screenPoint.z), _index);
			playerNamesObjs[_index].guiText.pixelOffset = new Vector2(screenPoint.x, screenPoint.y);
			playerNamesObjs[_index].guiText.fontSize = (int)(1000 / screenPoint.z);
		}
		else
		{
