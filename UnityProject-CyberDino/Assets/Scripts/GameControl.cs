	...
	private void InitializeGame(){
		...
		if (Network.isServer) {
			...
			foreach (var unit in turretSpawnPoints) {
					GameObject Turret = (GameObject)Network.Instantiate ((GameObject)Resources.Load (/*name of turret prefab*/"Turret"), unit.transform.position, Quaternion.LookRotation (unit.transform.forward), playerID);
			}

			GameObject[] pickupSpawnPoints = GameObject.FindGameObjectsWithTag ("PickUpSpawn");
			foreach (var unit in pickupSpawnPoints) {
					GameObject PickUp = (GameObject)Network.Instantiate ((GameObject)Resources.Load (/*name of pickup prefab*/"PickUp"), unit.transform.position, Quaternion.LookRotation (unit.transform.forward), playerID);
			}
